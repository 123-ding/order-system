...
| （三）警察/海关/应急管理部门驻地等上方≤120 m 飞行 | **无需** | **需要**（起飞前 1 小时） |
...
flowchart TD
    ...
    CHECK -->|情形一/二\n适飞空域/农用| EXEMPT([无需申请·无需确认\n可直接飞行])

    CHECK -->|情形三\n特定部门驻地≤120m| PRE3[计划起飞前 1 小时\n提交起飞前确认申请]
    PRE3 --> CONF3{"起飞前确认节点\n石狮市公安局值班席位（待确认）\n核验单位身份·区域高度·空域冲突"}
    CONF3 -->|确认通过| FLY3([允许起飞·留痕])
    CONF3 -->|不通过| REJ_3([拒绝·通知原因])

    CHECK -->|情形四\n机场管制地带内\n巡检/勘察/校验| PRE4[计划起飞前 1 小时\n提交起飞前确认申请\n前提：已完成定期备案]
    PRE4 --> CONF4{"起飞前确认节点\n石狮市公安局值班席位\n+机场管制部门联动（待确认）\n核验备案·机场协调·空域状态"}
    CONF4 -->|确认通过| FLY4([允许起飞·留痕])
    CONF4 -->|不通过| REJ_4([拒绝·通知原因])

    CHECK -->|需申请飞行活动\n一般/长期/紧急/场内| APPLY[提交飞行活动申请]
    APPLY --> N1{"审批节点①\n石狮市公安局\n低空飞行审批岗（待确认）\n受理·材料校验·合规审查"}
